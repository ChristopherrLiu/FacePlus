...
class ArcMargin(nn.Module) :
    def __init__(self, in_features, out_features, s=30.0, m=0.5) :
        ...
        self.cosine_m = math.cos(self.m)
        self.sine_m = math.sin(self.m)

    def forward(self, input, label) :
        cosine = F.linear(F.normalize(input), F.normalize(self.w0eight))
        sine = torch.sqrt(1.0 - torch.pow(cosine, 2))

        marginal_target_logit = cosine * self.cosine_m - sine * self.sine_m
        marginal_target_logit = torch.where(cosine > 0, marginal_target_logit, cosine)

        one_hot = torch.zeros(cosine.size(), device=self.weight.device)
        one_hot.scatter_(1, label.view(-1, 1).long(), 1)

        output = (one_hot * marginal_target_logit) + ((1.0 - one_hot) * cosine)
        output *= self.s
